...
SetupProducer = Callable[[RawRtpPeerConnection], Awaitable[ProducerTeardown]]
...
class RawRtpSfu:
	...
	def __init__(
		...
		*,
		...
		self._producer_task: asyncio.Task[ProducerTeardown] | None = None
	...
	def close_consumer(self, consumer: RawRtpPeerConnection) -> None:
		...
		if not self._consumers and self._producer_pc and not self._idle_task:
	...
	async def _ensure_producer(self) -> ProducerTeardown:
		if producer_task := self._producer_task:
		...
		producer_task = self._producer_task = asyncio.create_task(
			self._setup_producer_pc(producer_pc)
		)

		def reap_producer_task(task: asyncio.Task[ProducerTeardown]) -> None:
			current_producer = self._producer_task is task
			if task.cancelled():
				if current_producer:
				...
				return
			try:
				producer_teardown = task.result()
			except BaseException as err:
				self._logger.error("Error in raw RTP SFU producer: %s", err)
				if current_producer:
					self._producer_pc = None
					self._producer_task = None
					for consumer in tuple(self._consumers.values()):
		...
		producer_task.add_done_callback(reap_producer_task)
		return await asyncio.shield(producer_task)
	...
	async def _close_producer(self) -> tuple[BaseException | None, ...]:
		...
		producer_task, self._producer_task = self._producer_task, None
		...
		if producer_task:
			if producer_task.done():
				if not producer_task.cancelled():
					try:
						producer_teardown = producer_teardown or producer_task.result()
					except BaseException as err:
						results.append(err)
			else:
				producer_task.cancel()
				results.extend(await asyncio.gather(producer_task, return_exceptions=True))

		results.extend(await asyncio.gather(
			*(teardown() for teardown in (producer_teardown,) if teardown),
			*(closer.close() for closer in (producer_pc,) if closer),
			return_exceptions=True,
		))
