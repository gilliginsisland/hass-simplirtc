...
class RTCPeerConnection(AsyncIOEventEmitter):
    ...
    def __init__(self, configuration: Optional[RTCConfiguration] = None) -> None:
        ...
        self.__pendingRemoteCandidates: list[Optional[RTCIceCandidate]] = []
        self.__rawRtpRouter: Any | None = None
        self.__rawRtpPeerId: str | None = None
        self.__rawRtpIsOutput: bool | None = None
    ...
    def peer_id(self) -> str:
        """Return the raw RTP routing peer ID."""
        if self.__rawRtpPeerId is None:
            raise RuntimeError("Raw RTP peer has no ID")
        return self.__rawRtpPeerId
    ...
    def sctp(self) -> None:
        """
        SCTP data channels are not included in this vendored build.
        """
        return None
    ...
    async def addIceCandidate(self, candidate: Optional[RTCIceCandidate]) -> None:
        ...
        if self.__rawRtpRouter is not None and not self.__remoteDescription():
            self.__pendingRemoteCandidates.append(candidate)
            return
    ...
    async def add_pending_remote_candidates(self) -> None:
        """Apply raw RTP ICE candidates received before the remote description."""
        for candidate in self.__pendingRemoteCandidates:
            await self.addIceCandidate(candidate)
        self.__pendingRemoteCandidates = []
    ...
    async def createAnswer(self) -> RTCSessionDescription:
        ...
        for remote_m in self.__remoteDescription().media:
            if remote_m.kind in ["audio", "video"]:
                ...
                raise OperationError("Data channels are not supported by this vendored build")
    ...
    def createDataChannel(
        ...
        :rtype: None
        """
        raise NotImplementedError("Data channels are not supported by this vendored build")
    ...
    def setRawRtpRouter(
        ...
        *,
        ...
        """Enable raw RTP routing for this peer connection."""
        if self.__rawRtpRouter is not None:
            raise RuntimeError("Raw RTP router is already attached")
        if self.__transceivers or self.__sctp:
            raise RuntimeError("Raw RTP router must be attached before transceivers are created")
        self.__rawRtpRouter = router
        self.__rawRtpPeerId = peer_id
        self.__rawRtpIsOutput = is_output
    ...
    def __createDtlsTransport(self) -> RTCDtlsTransport:
        ...
        if self.__rawRtpRouter is not None:
            dtlsTransport = RTCDtlsTransport(
                iceTransport,
                self.__certificates,
                raw_rtp_router=self.__rawRtpRouter,
                raw_rtp_peer_id=self.peer_id,
                raw_rtp_is_output=bool(self.__rawRtpIsOutput),
            )
        else:
    ...
    def __createTransceiver(
        ...
        if self.__rawRtpRouter is not None:
            direction = "sendonly" if self.__rawRtpIsOutput else "recvonly"
            sender = RTCRtpSender(
