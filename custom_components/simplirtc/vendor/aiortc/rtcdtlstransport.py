...
class RTCDtlsTransport(AsyncIOEventEmitter):
    ...
    def __init__(
        ...
        *,
        ...
        self.__raw_rtp_is_output = raw_rtp_is_output
    ...
    async def _handle_rtcp_data(self, data: bytes) -> None:
        if self.__raw_rtp_router is not None and self.__raw_rtp_peer_id is not None:
            if self.__raw_rtp_is_output:
                await self.__raw_rtp_router.forward_output_rtcp(
                    self.__raw_rtp_peer_id,
                    data,
                )
            else:
                await self.__raw_rtp_router.forward_input_rtcp(
                    self.__raw_rtp_peer_id,
                    data,
                )
            return
    ...
    async def _handle_rtp_data(self, data: bytes, arrival_time_ms: int) -> None:
        if self.__raw_rtp_router is not None and self.__raw_rtp_peer_id is not None:
            if not self.__raw_rtp_is_output:
                await self.__raw_rtp_router.forward_input_rtp(
                    self.__raw_rtp_peer_id,
                    data,
                )
            return
